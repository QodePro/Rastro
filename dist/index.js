...
(function() {  // <-- Inicio de la IIFE -->
    ...
    const setupForm = ({ title, price }) => {
        ...
        const emailInput = $('pay__email');
        const textareaInput = $('pay__textarea');
        const buttonInput = $('pay__button');
        const paypalBtn = $('pay__paypal');
        const alertBtn = $('pay__alert');
        const input = $('pay__quantity');
        ...
        const validate = () => {
            const total = +(input?.value || 1) * +price.replace('$', '');
            const name = nameInput?.value?.trim() || '';
            const email = emailInput?.value?.trim() || '';
            const textarea = textareaInput?.value?.trim() || '';
            const details = title + input.value + total;
    
            if (formName) formName.value = name;
            if (formEmail) formEmail.value = email;
            if (formTextarea) formTextarea.value = details + textarea;
    
            const style = name && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)
                ? 'pointer-events: auto; opacity: 1'
                : 'pointer-events: none; opacity: 0.5';
    
            if (textareaInput) textareaInput.style.cssText = style;
            if (paypalBtn) paypalBtn.style.cssText = style;
            if (buttonInput) buttonInput.style.cssText = style;
        };

        const clearInputs = () => {
            const success = successMessage.innerText.trim();
            const error = errorMessage.innerText.trim();

            if (alertBtn) alertBtn.innerText = success || error || '';
            if (alertBtn) alertBtn.className = `is-active`;
    ...
    const setupPayp = ({ price, title }) => {
        ...
        const total = $('pay__total');
        const element = $('pay__paypal');
        const input = $('pay__quantity');
        const arlet = $('arlet');
        ...
        window.paypal.Buttons({
            ...
            onApprove: function(data, actions) {
                return actions.order.capture().then(function(details) {
                    ...
                    if (arlet) arlet.className = `is-active`;                 
                });
            },
    ...
    const contentHandler = (content, envio, {}) => {
        const handleContent = (cont, feed, type) => {
            ...
            return cont
                .replace(/<new-cont\.arlet>/g, '<p id="arlet"/>')
                .replace(/<new-cont\.pay>/g, '<div id="pay__paypal"/>')
                .replace(/<new-cont\.price>/g, '<span id="pay__total"/>')
                .replace(/<new-cont\.name>/g, '<input type="text" id="pay__name" placeholder="name *" />')
                .replace(/<new-cont\.email>/g, '<input type="email" id="pay__email" placeholder="email *" />')
                .replace(/<new-cont\.refer>/g, '<input type="text" id="pay__textarea" placeholder="Referido" />')
                .replace(/<new-cont\.quantity>/g, '<input type="number" id="pay__quantity" value="1" min="1"/>')
                .replace(/<new-cont\.back>/g, '<a onclick="history.back()">🡨 back</a>')
    ...
    const emailHandler = (content, envio, {}) => {
        const handleContent = (cont, feed, {}) => {
            ...
            return cont
                ...
                .replace(/<new-emal\.name>/g, '<input type="text" id="pay__name" placeholder="name *"/>')
                .replace(/<new-emal\.email>/g, '<input type="email" id="pay__email" placeholder="email *"/>')
                .replace(/<new-emal\.textarea>/g, '<textarea type="text" id="pay__textarea" placeholder="msj"></textarea>')
                .replace(/<new-emal\.button>/g, '<button type="btn" id="pay__button">Enviar</button>')
                .replace(/<new-emal\.arlet>/g, '<p id="pay__arlet"/>');
        };
        return processLoop(content, envio, handleContent);
    };
