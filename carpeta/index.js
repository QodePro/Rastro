...
(function() {  // <-- Inicio de la IIFE -->
    ...
    const setupForm = (info) => {
        ...
        const validate = () => {
            ...
            [nameInput, emailInput].forEach((input, i) => {
                const valid = [name, block.test(email)][i];
                if (input) input.classList.toggle('input--valid', valid);
        ...
        const clearInputs = () => {
            ...
            if (alertBtn) {
                ...
                alertBtn.className = success ? 'form--valid' : error ? 'form--error' : '';
            }
    ...
    const processLoop = (content, envio, handleContent) => {
        const processContent = (date) => (_, type, id, cont) => {
            const feed = date[type.split(':')[0]]?.feed;
            return handleContent(cont, feed, type, id);
        };
        return content
    ...
    const postHandler = (content, envio, No) => {
        const handleContent = (cont, feed, type, id) => {
            let [typ = '', category = '', max = ''] = type?.split(':') || [];
            console.log(typ, category, max);
    ...
    const pageHandler = (content, envio, {}) => {
        const handleContent = (cont, feed, type) => {
            ...
            const enlace = (page, num, isActive) =>
                ...
                .replace(/a class="([^"]*)"/, (_, p1) =>
                    `a class="${p1} labl--${dato[typ].label.replace(/ /g, '-')} ${isActive ? 'is&ndash;active' : ''}"`)
