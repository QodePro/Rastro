...
(function() {  // <-- Inicio de la IIFE -->
    ...
    const temp = (entry, limit) => {
        ...
        return {
            ...
            autor: `<img src='${entry?.author?.[0]?.gd$image?.src || DEFAULT}'>`,
    ...
    const contentHandler = (content, envio, {}) => {
        const handleContent = (cont, feed, type) => {
            ...
            setTimeout(() => setupPaypal(info), 0);
                
            return cont
                .replace(/<new-cont\.arlet>/g, '<p id="arlet"/>')
                .replace(/<new-cont\.pay>/g, '<div id="p1"/>')
                .replace(/<new-cont\.price>/g, '<span id="t1"/>')
                .replace(/<new-cont\.quantity>/g, '<input type="number" id="n1" value="1" min="1"/>')
