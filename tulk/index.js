...
(function() {  // <-- Inicio de la IIFE -->
    ...
    const rout = (key) => {
        ...
        const cleanPathname = pathname.replace("?m=1", "");
        const cleanSearch = search.replace("?m=1", "");
        const routeInfo = {
            tab: cleanPathname.split('/')[1] || '',
            search: new URLSearchParams(cleanSearch).get('q') || '',
            label: cleanPathname.match(/\/label\/([^/]*)/i)?.[1] || '',
            cont: cleanPathname.split('/')[2] || '',
            page: new URLSearchParams(cleanSearch).get('page') || '',
            rout: cleanPathname,
        };
    ...
    const setupPayp = ({ price = "$0", title = "" } = {}, base = {}) => {
        const $ = id => document.getElementById(id);
        const name = $('enlace-name');
        const email = $('enlace-email');
        ...
        window.paypal.Buttons({
            ...
            onApprove: (data, actions) => {
                return actions.order.capture().then(details => {
                    ...
                    if (alert) {
                        alert.className = 'pay--valid';
                        alert.innerHTML = `
                            <div class="icono"></div>
                            <div class="titulo">¡Gracias, ${name?.value?.trim() || "Usuario"}!</div>
                            <div class="subtitulo">Tu orden ha sido enviada exitosamente.</div>
                            <div class="info">
                                <div><strong>Correo:</strong> ${email?.value?.trim() || "N/A"}</div>
    ...
    function shortcodes(content) {
        ...
        content.replace(/@([a-zA-Z-]+)([\s\S]*?)@([a-zA-Z-]+)/g, (_, open, inners, close) => {
            let processed = '';
            let cleanInner = inners.replace(/<\/?(&nbsp;)\b[^>]*>/gi, '');
    ...
    const contentHandler = (content, envio) => {
        const handleContent = (cont, feed, type, base) => {
            ...
            htmlt = `
            <div class='sx-pays' id='cont'>
                <new-loop data='${type}' var='cont'>
                    <a class='sx-author' to='product.url'>${info.author}</a>
                    <div class='sx-items' id="enlace-bill">
                        <div class='sx-item'>
                            <div class='sx-image'>${info.image}</div>
                            <a class='sx-title' to='product.url'>${info.title}</a>
                            <input type="number" class='sx-quantity' id="enlace-quantity" value="1" min="1"/>
                            <span class='sx-price' id="enlace-total"></span>
                        </div>
                        <input type="text" class='sx-name' id="enlace-name" placeholder="name *" />
                        <input type="email" class='sx-email' id="enlace-email" placeholder="email *" />
                        <input type="text" class='sx-refer' id="enlace-textarea" placeholder="Referido" />
                        <div class='sx-paypal' id="enlace-paypal"></div>
                    </div>
                </new-loop>
            </div>
            `
    ...
    window.bs = { 
        ...
        data: (data = {}) => {
            Promise.all(Object.entries(data).map(([type, url]) => 
                new Promise((resolve, reject) => {
                    ...
                    script.src = `${cleanUrl}/feeds/posts/full?alt=json&callback=${type}`;
        ...
        pays: ({ baseUrl = "", style = {}, features = "", clientId = "" } = {}) => {
            const script = document.createElement("script");
            const url = `https://www.${baseUrl.replace(/^(https?:\/\/)?(www\.)?/, '')}`;
            script.src = `${url}/sdk/js?client-id=${clientId}&components=${features}`;
            document.head.appendChild(script);
            dase.pays = { style };
        },
        form: ({ formUrl = "" } = {}) => {
            const cleanUrl = formUrl.startsWith('http') ? formUrl : `https://${formUrl}`;
            dase.formUrl = cleanUrl;
        },
        test: (cb) => cb(dase)
    };
    ...
    window.addEventListener('DOMContentLoaded', tabs, cargar);
